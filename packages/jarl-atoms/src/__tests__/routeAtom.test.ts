...
import { rootRouteAtom, rootRoute } from "../rootRouteAtom";
...
describe("rootRoute", () => {
  it("matches the root path exactly by default", () => {
    ...
    const result = store.get(rootRoute);
  ...
  it("normalizes a trailing slash the same as no trailing slash", () => {
    ...
    // exercise rootRoute's normalization purely through the atoms' own
    ...
    const root = store.get(rootRoute);
    assertMatch(root);
...
describe("rootRouteAtom with basePath", () => {
  let store: ReturnType<typeof createStore>;
  ...
  it("matches at the base path itself", () => {
    const appRoot = rootRouteAtom({ basePath: "/app" });
    seed(store, "/app");
