...
import { Redirect, followRedirects, isRedirect, redirect, redirectRouteAtom } from "../redirectRouteAtom";
...
describe("redirectRouteAtom", () => {
  ...
  it("matches whenever its parent matches, static target", () => {
    ...
    const toHome = redirectRouteAtom("/", { parent: movedAtom });
    seed(store, "/moved");
  ...
  it("supports a dynamic target computed from other atoms", () => {
    const reasonAtom = queryParamRouteAtom("because");
    const movedAtom = staticRouteAtom("moved");
    const toHomeWithReason = redirectRouteAtom((get) => `/?because=${get(reasonAtom).values?.because ?? "unknown"}`, {
      parent: movedAtom,
    });
    seed(store, "/moved", "because=Permanently%20moved");
    expect(store.get(toHomeWithReason).reverse({})).toBe("/?because=Permanently moved");
  });
...
describe("followRedirects", () => {
  ...
  it("follows immediately if the redirect already matches when subscribed", () => {
    const banned = queryParamRouteAtom("banned", { required: true });
    const toBannedPage = redirectRouteAtom("/banned-page", { parent: banned });
    seed(store, "/", "banned=1");
