...
Route atoms are only jotai atoms, so none of this is a closed set: `useAtomValue(someRouteAtom)`
and `useSetAtom(someRouteAtom)` are always available, and anything else in jotai's ecosystem works
on them unchanged. `jarl-react` re-exports `useAtom`, `useAtomValue` and `useSetAtom` for exactly
that reason, so reaching past the named hooks doesn't mean adding a direct `jotai` dependency:
...
Beyond `route` and `to` (optional - it defaults to `{}`, for routes that bind no params), `Link`
takes:
...
underlying element as-is. When the markup isn't anchor-shaped at all, prefer `useLink` directly
over the render-prop form; it's the same three values without a component wrapped around them.
