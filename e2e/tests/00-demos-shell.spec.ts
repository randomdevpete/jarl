...
test.describe("Demos Shell", () => {
    test("loads home page", async ({ page }) => {
        ...
        await expect(page.locator("[data-test=version]")).toContainText(/^v/);
    });

    test("shows not found page", async ({ page }) => {
        await page.goto("/asdfghjkl");
        await expect(page).toHaveTitle(/Not Found/);
        await expect(page.locator("[data-test=header]")).toContainText("Not Found");
        await expect(page.locator("[data-test=body]")).toContainText("/asdfghjkl");
    });

    test("shows the changelog", async ({ page }) => {
        await page.goto("/changelog");
        await expect(page.locator("[data-test=content] h1")).toContainText("JARL: Version History");
    });
});
